...
stoi = { ch:i for i,ch in enumerate(chars) } # ch -> char, i -> index
...
# Splitting into training and testing sets (90/10)
n = int(0.9*len(data))
train_data = data[:n]
val_data = data[n:]
...
def get_batch(split):
    ...
    # Moving the data to device (GPU or CPU)
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        ...
        self.sa = MultiHeadAttention(n_head, head_size) # i.e. 4 heads of 8-dimensional self-attention # 'sa_heads' -> Short for SelfAttention_Heads
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.token_embedding_table = nn.Embedding(vocab_size, n_embd) # 'n_embd -> Short for Number_EmbeddingDimensions
        self.position_embedding_table = nn.Embedding(block_size, n_embd)
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head) for _ in range(n_layer)])
        self.ln_f = nn.LayerNorm(n_embd) # Final layer norm
        self.lm_head = nn.Linear(n_embd, vocab_size) # 'lm_head' -> Short for LanguageModeling_Head
    ...
    def forward(self, idx, targets=None):
        ...
        tok_emb = self.token_embedding_table(idx) # (B,T,C) -> (Batch, Time, Channel) Channel is vocab_size = 65
...
m = model.to(device) # # Moving the model parameters to device (GPU or CPU)
